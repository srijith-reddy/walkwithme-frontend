...
final class HazardFusion {
    ...
    // ============================================================
    // Backend → label set
    // Handles multiple response shapes from /vision:
    //   1. { "hazards": ["person", "car"] }                   — simple string array
    //   2. { "hazards": [{"label":"person","severity":"high"}] } — object array
    //   3. { "recommendations": ["watch for cyclists"] }       — text fallback (ignored for AR)
    //   4. { "analysis": "{...}" }                             — JSON-encoded string (unwrapped by ARSessionManager)
    // ============================================================
    private static func parseBackendLabels(_ json: [String: Any]) -> Set<String> {
        var labels: Set<String> = []

        // Shape 1 & 2: "hazards" key
        if let arr = json["hazards"] as? [Any] {
            for item in arr {
                if let s = item as? String {
                    labels.insert(s.lowercased())
        ...
        // Shape 3: "detections" key (some backends use this)
        if let arr = json["detections"] as? [[String: Any]] {
            for obj in arr {
