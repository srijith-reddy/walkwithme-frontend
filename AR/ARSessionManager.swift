...
final class ARSessionManager: NSObject, ObservableObject {
    ...
    /// Distance remaining to end of route (meters). Updated every HUD tick.
    @Published private(set) var distanceRemainingM: Double = 0
    @Published private(set) var pathStatusText: String?
    @Published private(set) var recommendationText: String?

    private override init() { super.init() }
}

extension ARSessionManager {
    /// Cleanly stop the session (call from AR view onDisappear).
    func stopSession() {
        hudTimer?.invalidate()
        hudTimer = nil
...
extension ARSessionManager {
    ...
    private func startTimers() {
        ...
        hudTimer = Timer.scheduledTimer(withTimeInterval: 0.15, repeats: true) { [weak self] _ in
            ...
            // Update distance remaining for AR bottom pill
            if let last = route.coordinatePoints.last {
                self.distanceRemainingM = distance(loc, last)
            }
    ...
    func uploadHazards(yolo: [YOLODetection]) {
        VisionUploader.shared.send(
            ...
            if case .success(let json) = res {
                ...
                let pathStatus = (obj["path_status"] as? String)?.trimmingCharacters(in: .whitespacesAndNewlines)
                ...
                DispatchQueue.main.async {
                    self.pathStatusText = pathStatus?.isEmpty == false ? pathStatus : nil
                    self.recommendationText = recommendation?.isEmpty == false ? recommendation : nil
